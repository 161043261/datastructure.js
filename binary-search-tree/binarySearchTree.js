  ...
  lowerBound(value, includeEqual = true) {
    ...
    const lowerBoundRecursive = (current) => {
      ...
      if (compare > 0 || (includeEqual && compare === 0)) {
        if (
          lowerBound === null ||
          this._compare(lowerBound.getValue(), current.getValue()) <= 0
        ) {
          lowerBound = current;
        }
  ...
  upperBound(value, includeEqual = true) {
    ...
    const upperBoundRecursive = (current) => {
      ...
      if (compare < 0 || (includeEqual && compare === 0)) {
        if (
          upperBound === null ||
          this._compare(upperBound.getValue(), current.getValue()) >= 0
        ) {
          upperBound = current;
        }
  ...
  removeNode(node) {
    ...
    if (node.isLeaf()) {
      if (node.isRoot()) {
        ...
        this._compare(node.getValue(), node.getParent().getValue()) < 0
      ) {
